(() => {
  ...
  function compactBlock(label, value, limit = 260) {
    const text = String(value || "-").trim() || "-";
    const safeLabel = escapeHtml(label);
    if (text.length <= limit) {
      return `<p class="compact-text"><strong>${safeLabel}:</strong> ${escapeHtml(text)}</p>`;
    }
    const preview = `${text.slice(0, limit).trim()}...`;
    return `<details class="compact-text"><summary><strong>${safeLabel}:</strong> ${escapeHtml(preview)}</summary><p>${escapeHtml(text)}</p></details>`;
  }
  ...
  async function loadUsers(force = false) {
    ...
    try {
      const [payload, overviewPayload] = await Promise.all([
        ...
        requestJson("/api/admin/overview").catch(() => null),
      ]);
      const items = Array.isArray(payload.items) ? payload.items : [];
      state.totals.users = Number(payload.total || 0);
      state.hasMore.users = Boolean(payload.has_more);
      const summary = overviewPayload && typeof overviewPayload === "object" ? overviewPayload.summary || {} : {};
      if (refs.usersSummary) {
      ...
      if (!items.length) {
        ...
        refs.usersList.innerHTML = items.map((row) => {
          ...
          const actionButtons = row.is_blocked
            ...
            : `<button class="btn small btn-danger user-action-btn" data-user-id="${escapeHtml(row.telegram_id)}" data-action="block">Mute/Block</button>`;
          ...
          return `<article class="list-item compact-user${selectedClass}" data-user-id="${escapeHtml(row.telegram_id)}"><div class="row-head"><h4>${escapeHtml(userLabel(row))}</h4><span class="chip ${chipClass}">${chipLabel}</span></div><div class="list-meta"><span>ID: ${escapeHtml(row.telegram_id || "-")}</span><span>Started: ${row.has_started ? "yes" : "no"}</span><span>Last: ${escapeHtml(formatDateTime(row.last_seen_at))}</span><span>Msgs: ${escapeHtml(formatNumber(row.total_messages))}</span><span>Imgs: ${escapeHtml(formatNumber(row.total_images))}</span></div><div class="toolbar wrap compact-actions">${openButton}${warnButton}${messageButton}${actionButtons}${kickButton}</div>${row.last_delivery_error ? `<p class="error">Delivery: ${escapeHtml(row.last_delivery_error)}</p>` : ""}</article>`;
        }).join("");
      }
    ...
    try {
      ...
      if (!items.length) {
        ...
        refs.conversationsList.innerHTML = items.map((row) => {
          ...
          return `<article class="list-item conversation-item"><div class="row-head"><h4>${escapeHtml(userLabel(row))}</h4><button class="btn small copy-btn" data-copy="${escapeHtml(copyPayload)}">Copy</button></div>${compactBlock("User", row.user_message || row.text_content || "-")}${compactBlock("Assistant", row.bot_reply || "-")}${renderMediaBlock(row, "Conversation media")}<div class="list-meta"><span>${escapeHtml(formatDateTime(row.created_at))}</span><span>Feature: ${escapeHtml(row.feature_used || "-")}</span><span>Type: ${escapeHtml(row.message_type || "-")}</span><span>Status: ${row.success ? "success" : "failed"}</span></div>${row.media_error_reason ? `<p class="error">Media issue: ${escapeHtml(row.media_error_reason)}</p>` : ""}</article>`;
        }).join("");
      }
